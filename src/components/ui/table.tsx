...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
    return <thead data-slot="table-header" className={cn("bg-primary-600 [&_tr]:border-b", className)} {...props} />;
}
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
    return (
        <th
            ...
            className={cn("h-10 px-2 text-left align-middle font-medium whitespace-nowrap text-white [&:has([role=checkbox])]:pr-0", className)}
            {...props}
        />
    );
}
